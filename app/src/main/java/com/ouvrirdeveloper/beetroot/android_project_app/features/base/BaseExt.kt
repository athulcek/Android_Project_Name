package com.ouvrirdeveloper.beetroot.android_project_app.features.base
...
import com.ouvrirdeveloper.domain.models.common.*
        ...
        if (!errorHandler.invoke()) {
            if (it is Resource.Error) {
                when (it.erroStatus) {
                    ...
                    is HTTP_ERROR -> {
                        val errorCode = (it.erroStatus as HTTP_ERROR).errorCode
                        if (errorCode == 401 || errorCode == 403) {
                            userUseCase.logoutUserUseCase()
